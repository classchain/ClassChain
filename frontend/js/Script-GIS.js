...
const basemapLayers = {
    carto: L.tileLayer('https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png', {
        attribution: '© CartoDB', maxZoom: 20, subdomains: 'abcd', crossOrigin: true
    }),
    persiangis: L.tileLayer('https://map.persiangis.ir/tile/{z}/{x}/{y}.png', {
        attribution: '© PersianGIS', maxZoom: 19, crossOrigin: true
    }),
...
window.addEventListener('resize', () => {
    clearTimeout(window.__gisMapResize);
    window.__gisMapResize = setTimeout(refreshMapSize, 100);
});
...
fetch('data/Projects.json').then(r => r.json()).then(data => {
    data.features.forEach(feature => {
        ...
        marker.on('click', function(e) {
            ...
            const fundsInfo = buildFundsHtml(a);
            enableDonateContext(a.project_id || a.OBJECTID || a.id, fundsInfo.primaryAddress);

            let financialInfo = '';
            if (fundsInfo.hasTreasury) {
                financialInfo = `
                    ...
                    <div class="info-item" style="margin-top:15px;">
                        <span class="info-label">وضعیت:</span><span class="info-value">${a.status || '—'}</span>
                    </div>
            ...
            showInPanel(`
                <div class="project-info">
                    <h3>${a.name || a.Name || 'پروژه'}</h3>
                    ${a.province || a.Province ? `<div class="info-item"><span class="info-label">استان:</span><span class="info-value">${a.province || a.Province}</span></div>` : ''}
                    ${a.county || a.County ? `<div class="info-item"><span class="info-label">شهرستان:</span><span class="info-value">${a.county || a.County}</span></div>` : ''}
                    ${financialInfo}
                </div>`, 'full');

            focusLatLngInVisibleMap(this.getLatLng(), 14);

            if (fundsInfo.hasTreasury && typeof window.loadProjectRaised === 'function') {
                window.loadProjectRaised(a);
            }
            if (fundsInfo.hasTreasury && typeof window.loadProjectDonors === 'function') {
                window.loadProjectDonors(a);
            }
        });
...
function showCountiesOfProvince(provinceName) {
    ...
    fetch('data/ir-counties.json').then(r => r.json()).then(data => {
        const features = data.features
            .filter(f => (f.attributes.pname || f.attributes.Pname) === provinceName)
            .map(f => ({
                type: 'Feature',
                geometry: { type: 'Polygon', coordinates: f.geometry.rings },
                properties: f.attributes
            }));
        const capitas = features.map(f => f.properties.C_capita || f.properties.capita || 0).filter(c => c > 0);
        const minCapita = capitas.length ? Math.min(...capitas) : 1;
        const maxCapita = capitas.length ? Math.max(...capitas) : 1;
        ...
        countiesLayer = L.geoJSON(features, {
            style: feature => {
                const c = feature.properties.C_capita || feature.properties.capita || 0;
            ...
            onEachFeature: (feature, layer) => {
                ...
                layer.on('click', e => {
                    ...
                    showInPanel(`
                        <div class="province-info">
                            <h3>شهرستان ${c.Name || c.name || 'نامشخص'}</h3>
                            ${c.pname || c.Pname ? `<div class="info-item"><span class="info-label">استان:</span><span class="info-value">${c.pname || c.Pname}</span></div>` : ''}
                        </div>`);
                    fitLayerToVisibleMap(layer, { duration: 0.9 });
                });
            }
        });

        features.forEach((f, i) => {
            const layer = countiesLayer.getLayers()[i];
            if ((c.pname || c.Pname) === provinceName) layer.addTo(map);
        });

        // simpler: add whole layer
    ...
    }).catch(err => console.error('counties', err));
}
...
function redirectToDonate() {
    ...
    q.set('project', currentProjectId);
    if (currentContractAddress) q.set('contract', currentContractAddress);
    window.location.href = 'donate.html?' + q.toString();
}

if (contributeActionBtn) {
    contributeActionBtn.addEventListener('click', redirectToDonate);
}
